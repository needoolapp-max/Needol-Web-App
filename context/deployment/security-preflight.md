...
- Admin panel access is now intended to use Clerk, with backend-side email allowlist enforcement.
...
- Set `CLERK_SECRET_KEY` on Render only.
- Set `ADMIN_ALLOWED_EMAILS` on Render to the single approved admin Gmail.
- Set `VITE_CLERK_PUBLISHABLE_KEY` on the admin panel.
- Set `VITE_ADMIN_ALLOWED_EMAILS` on the admin panel only as a UI-side convenience, not as the source of truth.

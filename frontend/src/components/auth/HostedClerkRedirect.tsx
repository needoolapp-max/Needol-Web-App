...
import { getClerkHostedAuthUrl, type ClerkAuthKind } from "@/lib/clerk-hosted-auth";
...
export function HostedClerkRedirect({ embeddedHref, kind }: HostedClerkRedirectProps) {
  ...
  const hostedUrl = useMemo(() => getClerkHostedAuthUrl(kind), [kind]);
  ...
  useEffect(() => {
    ...
    if (!hostedUrl) {
      recordDashboardEvent("auth:hosted-url-missing", { kind });
      setRedirectFailed(true);
      return;
    }

    recordDashboardEvent("auth:hosted-redirect", { kind, host: new URL(hostedUrl).host });
    const fallback = window.setTimeout(() => setRedirectFailed(true), 2500);
    window.location.replace(hostedUrl);
    return () => window.clearTimeout(fallback);
  }, [hostedUrl, isLoaded, isSignedIn, kind, navigate, user?.id]);
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-6 text-center shadow-sm">
      ...
      <p className="mt-2 text-sm leading-6 text-muted-foreground">
        You will continue on Clerk's hosted auth page, then return to your dashboard.
      </p>
      ...
      {redirectFailed && (
        <p className="mt-4 text-xs leading-5 text-destructive">
          Automatic redirect did not start. Use the secure continue button, or set the Clerk Account
          Portal URL in the deployment environment.
        </p>
      )}
    </div>
  );
